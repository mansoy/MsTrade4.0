...
{$IFDEF VER210}
  ...
  TSuperRttiContext = class
  private
    class function GetFieldName(r: TRttiProperty): string;
    class function GetFieldDefault(r: TRttiProperty; const obj: ISuperObject): ISuperObject;
  public
...
class function TSuperRttiContext.GetFieldName(r: TRttiProperty): string;
var
  o: TCustomAttribute;
begin
...
class function TSuperRttiContext.GetFieldDefault(r: TRttiProperty; const obj: ISuperObject): ISuperObject;
var
  o: TCustomAttribute;
begin
...
function TSuperRttiContext.FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject;
      ...
      stObject:
        begin
          ...
          for f in Context.GetType(Value.AsObject.ClassType).GetProperties do
            ...
            begin
              Result := FromJson(f.PropertyType.Handle, GetFieldDefault(f, obj.AsObject[GetFieldName(f)]), v);
              if Result then
  ...
  begin
    Result := True;
    TValue.Make(nil, TypeInfo, Value);
    for f in Context.GetType(TypeInfo).GetProperties do
    begin
      if ObjectIsType(obj, stObject) and (f.PropertyType <> nil) then
      begin
  ...
  begin
    ...
    if CompareMem(@vGuid, @soguid, SizeOf(TGUID)) then
    begin
      if obj <> nil then
...
function TSuperRttiContext.ToJson(var value: TValue; const index: ISuperObject): ISuperObject;
  ...
  begin
    Result := TSuperObject.Create(stObject);
    for f in Context.GetType(Value.TypeInfo).GetProperties do
    begin
